...
const Card = (data) => {
    ...
    const renderIcon = (id) => {
        ...
        if (isInCart) {
            return (
                <div className="absolute top-0 right-0 flex justify-center items-start bg-black w-6 h-6 rounded-full m-2">
                    <CheckIcon className="size-6 text-white"></CheckIcon>
                </div>
            )
        } else {
            return (
                <div className="absolute top-0 right-0 flex justify-center items-start bg-white w-6 h-6 rounded-full m-2" onClick={(event) => addProductsToCart(event, data?.data)}>
